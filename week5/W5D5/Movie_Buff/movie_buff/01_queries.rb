...
def harrison_ford
  ...
  Movie
    .select(:id,:title)
    .joins(:actors) #attaching an association
    .where(actors: {name:'Harrison Ford' }) #Referencing a table
    .where.not(castings:{ord:1}) #associating castings table
...
def biggest_cast
  ...
  #Movie->:id, :title, :yr, :score, :votes, :director_id
  Movie
    .select(:id, :title)
    .joins(:actors)
    .group('movies.id')
